...
valStore (Ptr typ loc) val = do
    (flip assert) "underlying types don't match" =<< typesMatch typ (valType val)
    void $ case val of
...
valArrayIdx idx val@(Val (Array n t) op) = do
    new@(Ptr _ _) <- valLocal (Array n t)
    valStore new val
    valArrayIdx idx new
...
valArrayConstIdx (Ptr (Array n t) loc) i =
    fmap (Ptr t) (gep loc [int64 0, int64 (fromIntegral i)])
...
valTupleIdx (Ptr (Tuple ts) loc) i = do
    assert (i >= 0 && i < length ts) "tuple index out of range"
    fmap (Ptr (ts !! i)) (gep loc [int64 0, int64 (fromIntegral i)])
