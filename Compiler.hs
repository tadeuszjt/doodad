...
cmpTopStmt (S.Assign pos pattern expr) = do
    ...
    where
        ...
        assignPattern pos (S.PatIdent _ symbol) val@(typ, op) = do
            checkUndefined pos symbol
            name <- freshName (mkBSS symbol)
            loc <- valGlobalClone pos name val
            opTyp <- opTypeOf pos typ

            addDef name (globalDef name opTyp Nothing)
            addSymObj symbol KeyVal $ ObjVal (typ, loc)
            addSymObjRequirement symbol KeyVal name
            addDeclared name
            addExported name
...
cmpStmt (S.Print pos exprs) = do
    ...
    where
        prints :: [Value] -> Instr ()
        prints []     = return ()
        prints [val]  = valPrint val
        prints (v:vs) = valPrint v >> printf ", " [] >> prints vs


cmpExpr :: S.Expr -> Instr Value
cmpExpr (S.Call pos symbol args) = do
    (argTypes, argOps) <- fmap unzip (mapM cmpExpr args)
    ObjFunc retType op <- look pos symbol (KeyFunc argTypes)
...
cmpExpr expr = case expr of
    ...
    S.Ident pos symbol -> do
        ObjVal (typ, loc) <- look pos symbol KeyVal
        op <- load loc 0
        return (typ, op)
...
--        unless (isExpr newType) $ cmpErr pos ("isn't expression type")
--        (typ, op) <- cmpExpr expr
--        unless (isExpr newType) $ cmpErr pos ("isn't an expression")
...
--        unless (isExpr elemType) (cmpErr pos "element type isn't expression")
...
--    unless (isExpr retType) (cmpErr pos "return isn't expression type")
--    retOpType <- opTypeOf retType
--
--    (paramTypes, paramOpTypes) <- fmap unzip $ forM params $ \(S.Param pos name typ) -> do
--        paramType <- fromASTType pos typ
--        unless (isExpr paramType) (cmpErr pos "param isn't an expression")
...
--        unless (isExpr typ) (cmpErr pos "param type isn't an expression")
--        return typ
--    paramOpTypes <- mapM opTypeOf paramTypes
--    
--    let key = KeyFunc paramTypes
--    entry <- lookupSymKey symbol key
--    when (isJust entry) $ cmpErr pos (symbol ++ "already defined")
--
...
--    addSymObjRequirement symbol (KeyFunc paramTypes) name
--
