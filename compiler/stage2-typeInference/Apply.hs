...
applyStmt subs stmt = case stmt of
    ...
    Let pos pattern expr mblk -> Let pos (applyPat pattern) (applyEx expr) (fmap applySt mblk)
...
applyExpr subs expression = case expression of
    ...
    Call pos params symbol args -> Call pos (map applyEx params) symbol (map applyEx args)
