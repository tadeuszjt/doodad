...
collectAST ast = do
    forM (Map.toList $ typeFuncs ast) $ \(symbol, (argSymbols, typ)) -> do
        collectTypeFunc symbol argSymbols typ
...
collectTypeFunc symbol argSymbols typ = do
    define symbol KeyType (ObjTypeFunc argSymbols typ)
-- TODO do i need this
--    case typ of
